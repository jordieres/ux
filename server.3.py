...
def check_agents(machine, platform, password,my_dict):
    if platform != DEFAULT_pltfrm:
        cmd  = " ps aux |grep python3 |grep log.py | grep " + platform + " | wc -l"
        ...
        #
        cmd  = " ps aux |grep python3 |grep browser.py | grep " + platform + " | wc -l"
        oshl = 'rsh '+str(machine) + cmd
        out_1= subprocess.Popen(oshl, stdout=subprocess.PIPE, stdin=None, stderr=None, \
                            close_fds=True, shell=True, universal_newlines = True)
...
def list_coils_orders_active(pltf, log, browser, launcher, my_dict, password,loc=0):
    n_orders = 0
    n_coils  = 0
    lst_coils= pd.DataFrame(columns=['pid','idcoil', 'plans', 'width', 'mat', \
...
def launch_plant(plant, machine, my_dict, platform, log, browser, password):
    ...
    if plant not in st.session_state['mchnvec_plnt'][platform]['plnts'][machine]:
        if platform != DEFAULT_pltfrm:
            ...
            fp.close()
            subprocess.call(['chmod','0755',path])
            oshl = " scp -p "+path+" @"+str(machine)+":\$HOME/"+my_dict+"/lchorden.sh"
            err0 = subprocess.Popen(oshl, stdout=subprocess.PIPE, stdin=None, \
                                stderr=subprocess.PIPE, close_fds=True, shell=True)
            oshl1= 'rsh '+str(machine)+ ' "sh ' + str(my_dict) + '/lchorden.sh"'
            time.sleep(1)
...
def main():
    ...
    optplnts  = (DEFAULT_plnts,'VA08','VA09','VA10','VA11','VA12')
    #
    st.empty()
    ...
    if current_tab == 'PLANTS':
        ...
        if plntsel != DEFAULT_plnts and plntsel != st.session_state['plntsel']:
            st.session_state['plntsel'] = plntsel
        if plntsel in st.session_state['mchnvec_plnt'][platform]['plnts'][plant_mchn]:
            dir_plnts   = st.session_state['mchnvec_ordr'][platform]['dirs'][plant_mchn]
            pltsec      = st.empty()
            pltsec.write("Welcome to the Plant " + plntsel + ". Information Section")
            boton       = right_plnt.button('Refresh', key = 'refresh1')
            plant_df    = execute_plant(plntsel,pltsec,plant_mchn,dir_plnts,\
                                        platform,password)
            pltsec.dataframe(plant_df)
        #
        #---> REPENSAR
    if shutdown:
        #eliminar carpetas?
